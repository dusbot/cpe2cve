...
func (a Attributes) ToCpe22String() string {
	var parts []string
...
func UnbindURI(s string) (*Attributes, error) {
	...
	s = strings.ToLower(s[len(uriPrefix):])
	attr := Attributes{}
	var err error
	...
	for ; i < len(s); i++ {
		...
		case '%':
			...
			if code == 0x1 || code == 0x2 {
				if !(i == at || i == len(s)-3 || s[i+3] == till ||
					(!embedded && i > 2 && s[i-3:i] == codeStr ||
						(embedded && i+6 < len(s) && s[i+3:i+6] == codeStr))) {
